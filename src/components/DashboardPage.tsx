...
// This replaces the current filteredRides useMemo hook in the component
  ...
  const renderImportantRide = (ride: Ride) => {
    ...
    return (
      <Link href={`/rides/${ride.id}?from=active`} key={ride.id}>
        <Card
          className={`mb-4 hover:bg-accent/50 transition-all duration-200 group border-l-4 hover:shadow-md 
            ${ride.status === "completed"
              ? "border-l-blue-500"
            ...
            <div className="flex flex-col space-y-4">
              ...
              <div className="flex items-start justify-between">
                ...
                <div className="flex flex-col items-end space-y-1">
                  <StatusBadge status={ride.status} />
                  {actionNeeded && <Badge variant="destructive" className="text-xs">{actionNeeded}</Badge>}
                </div>
              </div>
              ...
              {(ride.note || ride.rider_phone) && (
                <>
                  ...
                  <div className="bg-accent/50 rounded-lg p-3 space-y-2 text-sm">
                    ...
                    {ride.rider_phone && (
                      <div className="flex items-center justify-between">
                        <span className="font-medium mr-2">Rider Phone:</span>
                        <span className="text-muted-foreground">{ride.rider_phone}</span>
                      </div>
                    )}
                  </div>
                </>
  ...
  return (
    <div className="mb-6 bg-accent/20 rounded-lg p-4 border border-accent/30">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h2 className="text-xl font-bold">Important Rides</h2>
      ...
      <ScrollArea className="w-full max-h-[350px] sm:max-h-[450px]">
