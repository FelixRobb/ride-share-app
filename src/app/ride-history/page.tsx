...
import { Search, Clock, MapPin, User2, CalendarIcon, ArrowRight, CheckCircle, X, Loader2, CircleSlash, ArrowBigLeft } from 'lucide-react'
    ...
    const fetchRides = useCallback(
        async (resetRides = false) => {
            ...
            try {
                ...
                setRides(prevRides => (resetRides ? data.rides : [...prevRides, ...data.rides]))
                setHasMore(data.hasMore)
                setTotalCount(data.totalCount)
                if (resetRides) setPage(1)
            } finally {
                setIsLoading(false)
            }
        },
                ...
                <div className="container mx-auto px-4 py-8">
                    ...
                    <div className="mb-6 px-4 py-2 bg-muted rounded-md flex justify-between items-center">
                        <span className="text-sm">
                            Showing <span className="font-medium">{rides.length}</span> of <span className="font-medium">{totalCount}</span> rides
                        </span>
                        {filterCount > 0 && (
