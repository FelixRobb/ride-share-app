...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      const { data: userContacts, error: userContactsError } = await supabase.from("contacts").select("contact_id, user_id").or(`user_id.eq.${userId},contact_id.eq.${userId}`).eq("status", "accepted");

      if (userContactsError) throw userContactsError;

      const contactIds = userContacts.map((contact) => (contact.contact_id === userId ? contact.user_id : contact.contact_id));
  ...
  try {
    ...
    try {
      ...
      for (const contact of contacts) {
        await sendImmediateNotification(contact.contact_id, "New Ride Available", `A new ride is available from your contact ${currentUser.name}`);
