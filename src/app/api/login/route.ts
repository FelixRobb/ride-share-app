...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (loginMethod === "email") {
      ...
      return NextResponse.json({ error: "Invalid login method", code: "INVALID_LOGIN_METHOD" }, { status: 400 });
    }
    ...
    if (!user.is_verified) {
      return NextResponse.json({ error: "Email not verified", code: "EMAIL_NOT_VERIFIED", userId: user.id }, { status: 403 });
    }
